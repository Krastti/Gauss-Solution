...
static void write_timing_row(FILE* out, size_t n, const char* method, int status, int ok_status, double total_ms, double decompose_ms, double solve_ms, const double* A, const double* b, const double* x) {
    const double residual = status == ok_status ? gauss_residual(n, A, b, x) : -1.0;

    fprintf(out, "%zu,%s,%d,%.6f,%.6f,%.6f,%.12e\n",
            n, method, status, total_ms, decompose_ms, solve_ms, residual);
}

static void write_accuracy_row(FILE* out, size_t n, const char* method, int status, int ok_status, const double* A, const double* b, const double* x, const double* exact) {
    const double relative_error = status == ok_status ? matgen_relative_error(n, x, exact) : -1.0;
    const double residual = status == ok_status ? gauss_residual(n, A, b, x) : -1.0;
...
static void write_gauss_solution(FILE* out, size_t n, const char* method_name, int method, const double* A, const double* b, double* x) {
    ...
    write_timing_row(out, n, method_name, status, GAUSS_OK, elapsed, 0.0, elapsed, A, b, x);
}
...
static void write_lu_solution(FILE* out, size_t n, const double* A, const double* b, double* x) {
    ...
    write_timing_row(out, n, "lu", status, LU_OK, total_ms, decompose_ms, solve_ms, A, b, x);
}
...
static void write_gauss_accuracy(FILE* out, size_t n, const char* method_name, int method, const double* A, const double* b, double* x, const double* exact) {
    ...
    write_accuracy_row(out, n, method_name, status, GAUSS_OK, A, b, x, exact);
}
...
static void write_lu_accuracy(FILE* out, size_t n, const double* A, const double* b, double* x, const double* exact) {
    ...
    write_accuracy_row(out, n, "lu", status, LU_OK, A, b, x, exact);
}
...
static void write_single_system_experiment(FILE* out) {
    ...
    for (size_t i = 0; i < ARRAY_COUNT(sizes); i++) {
        ...
        if (!A || !b || !x) {
            ...
            fprintf(out, "%zu,allocation,error,0,0,0,0\n", n);
            gauss_free_matrix(A);
            free(b);
            free(x);
            continue;
...
static void write_multiple_rhs_experiment(FILE* out) {
    ...
    if (!A || !L || !U || !x) {
        ...
        fprintf(out, "%zu,0,allocation,error,0,0,0\n", n);
        gauss_free_matrix(A);
        gauss_free_matrix(L);
        gauss_free_matrix(U);
        free(x);
    ...
    for (size_t i = 0; i < ARRAY_COUNT(rhs_counts); i++) {
        ...
        for (size_t rhs = 0; rhs < k; rhs++) {
            ...
            if (!b) {
                ...
                gauss_status = GAUSS_ERROR;
                solve_status = LU_ERROR;
                break;
            }
...
static void write_hilbert_experiment(FILE* out) {
    ...
    for (size_t i = 0; i < ARRAY_COUNT(sizes); i++) {
        ...
        if (!H || !exact || !x) {
            ...
            fprintf(out, "%zu,allocation,error,-1,-1\n", n);
            gauss_free_matrix(H);
            free(exact);
            free(x);
            continue;
...
int experiments_run_all(void) {
    LOG_INFO("%s", "experiments_run_all: запуск экспериментов лабораторной работы");
    ensure_report_directories();

    LOG_INFO("%s", "experiments_run_all: открытие CSV-файлов отчётов");
    ...
    LOG_INFO("%s", "experiments_run_all: все отчёты успешно записаны");
    return 0;
}
